...
#define PU_SETSIZECONSTRAINT(pctxt,rootLower,rootUpper,constrExtensible) \
if (!ACINFO(pctxt)->sizeConstraint.isSet) { \
ACINFO(pctxt)->sizeConstraint.root.lower = rootLower; \
ACINFO(pctxt)->sizeConstraint.root.upper = rootUpper; \
ACINFO(pctxt)->sizeConstraint.isSet = TRUE; \
ACINFO(pctxt)->sizeConstraint.extensible = constrExtensible; }

/* This macro initializes the size constraint in the context */

#define PU_INITSIZECONSTRAINT(pctxt) \
ACINFO(pctxt)->sizeConstraint.root.lower = 0; \
ACINFO(pctxt)->sizeConstraint.root.upper = 0; \
ACINFO(pctxt)->sizeConstraint.isSet = FALSE; \
ACINFO(pctxt)->sizeConstraint.extensible = FALSE
 ...
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue      Receives pointer to decoded value, represented as a
 *                      dotted number string (e.g. "0.5.1234"). The caller
 *                      is responsible for freeing this memory (e.g. with
 *                      rtxMemFreePtr).
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_ObjectIdentifierStr(OSCTXT* pctxt, char** pvalue);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_RelativeOIDStr(OSCTXT* pctxt, char** pvalue);
...
/**
 * This function encodes a value of the ASN.1 object identifier type,
 * provided as a dotted number string (e.g. "0.5.1234").
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue
 * @return            Completion status of operation:
 ...
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue      Pointer to value to be encoded, represented as a
 *                       dotted number string (e.g. "0.5.1234").
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pe_RelativeOIDStr(OSCTXT* pctxt, const char* pvalue);
 ...
 * @param pExtSize     Receives true if value is extension value and false
 *                      if a root value.
 * @return             Status of the operation.  0 = success or RTERR_CONSVIO
 *                       if size if not within the root and the constraint is
 *                       not extensible.
 */
EXTERNPER int pu_checkSizeExt
(Asn1SizeCnst* pSizeCnst, OSSIZE value, OSBOOL* pExtSize);
