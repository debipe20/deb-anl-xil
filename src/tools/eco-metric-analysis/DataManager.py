...
class DataManager:
    ...
    def plotRelativeDistanceAndSpeedProfile(self):
        ...
        for index, leadVehicleLogFile in enumerate(self.ecoDrivingLeadVehicleLogFileList):
            # print("File name is:", leadVehicleLogFile )
            logFileName = '/nojournal/bin/log/eco-driving/' + leadVehicleLogFile +'.csv'
            ...
            plt.tight_layout()
            plt.savefig("diagram/eco-driving/" + leadVehicleLogFile +'-plot.jpg', bbox_inches='tight', dpi=72)
            # plt.show()
