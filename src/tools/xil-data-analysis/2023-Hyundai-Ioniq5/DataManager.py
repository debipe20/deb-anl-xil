    ...
    def get_accel_decel_time(self):
        ...
        for index, value in enumerate(self.accel_data_rqst):
            ...
            if (test_start_status and accel_value != self.accel_data_rqst[index] and accel_rqst_start_time == 0 and
                ...
                # Iterate from the current index until the acceleration value changes to find the minimum speed.
                df_slice = self.analyzed_data_frame.loc[index:]
                for idx, row in df_slice.iterrows():
                    # Stop if the acceleration value changes
                    if row["Acceleration Requested [mps2]"] != accel_value:
                        break
                    # Stop if a specific speed threshold is reached
                    if row["Speed [mph]"] <= self.starting_speed:
                        min_speed = row["Speed [mph]"]
                        break
                    # Update the minimum speed observed so far
                    min_speed = min(min_speed, row["Speed [mph]"])
                            
                if accel_value > 0 and (max_speed > self.ending_speed):
                    max_speed = self.ending_speed
                    
                elif accel_value < 0 and (min_speed < self.starting_speed):
                    min_speed = self.starting_speed
