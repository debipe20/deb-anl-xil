...
class Decoder:
    ...
    def _tokenize_msg_from_bits(self, msg, start_bit, length, factor, offset, byte_order, gamma):
        ...
        if byte_order == 'be': 
            ...
            signal = Signal(start_bit, length, byte_order=byte_order, classification='ts', msg=msg, name=signal_name, factor=factor, offset=offset)
    ...
    def _tokenize_msg(self, msg, byte_order, tokenization_method, signedness_method, alpha1, alpha2, gamma):
        ...
        for i in range(range_start, range_end):
            # At end of message, end of signal determined by method, or end of constant signal
            if (i == range_end-1) or \
    ...
    def plot_signal(self, signal, message_start):
        '''
        Plot time series signal value
        ...
        plot_ts_signal(ax, signal, message_start)
        plt.show()
    ...
    def find_signal_match(self, signal_b, num_matches=5, thresh=0.5, print_sig=False, plot=False, start_time=0.0, end_time=10**10):
        '''
        Loop through all signals and find matches to input signal

            Parameters:
                signal_b (PandaCANDecoder.signal.Signal())
                num_matches (int): number of matched signals to plot
                thresh (float): print and plot signals with R^2 scores above this threshold
        '''
        match_found = True
        top_matches = []
        for msg in self.msgs:
            for signal_a in msg.signals:
                if signal_a.msg.msg_id == signal_b.msg.msg_id or signal_a.classification == 'constant':
                    continue
                model, r_sq, _ = self.signal_match(signal_a, signal_b, start_time, end_time)
                if r_sq > thresh:
                    if print_sig:
                        print(signal_a, " | R^2: ", r_sq)
                    top_matches.append((signal_a, signal_b, model, r_sq))

        if plot:
            top_matches = sorted(top_matches, key=lambda x: x[-1], reverse=True)
            for i in range(num_matches):
                if len(top_matches) > i:
                    print(top_matches[i])
                    self.plot_signal_match(top_matches[i][0], top_matches[i][1], top_matches[i][2], top_matches[i][3])
                else:
                    print(f'No more matches for signal {signal_b} above threshold {thresh}')
                    match_found = False
                    break
            
        if not match_found:
            return self.segment_signal_match(signal_b, num_matches, thresh, plot=plot)
            
        return top_matches[:num_matches]
    
    def segment_signal_match(self, signal_b, num_matches, thresh, plot=False):
        '''
        If no signals match, try to match segments of the signal
            ...
            Parameters:
                ...
                thresh (float): print and plot signals with R^2 scores above this threshold
        '''
        print('Other potential matches include:')
        timestamp_diff = np.diff(signal_b.msg.ts_data[:,0])
        timestamp_segments = []
        message_segments = []
        time1 = 0
        for time2, diff in enumerate(timestamp_diff):
            if diff >= 5e9:
                timestamp_segments.append(signal_b.ts_data_timestamps[time1:time2])
                message_segments.append(signal_b.ts_data[time1:time2])
                time1 = time2 + 1
        ...
        for msg in self.msgs:
            for signal_a in msg.signals:
                ...
                total_points = 0
                segment_points_list = []
                for i in range(len(timestamp_segments)):
                    start_time = timestamp_segments[i][0]
                    end_time = timestamp_segments[i][-1]
                    model, r_sq, segment_points = self.signal_match(signal_a, signal_b, start_time, end_time)
                    total_points += segment_points
                    segment_points_list.append(segment_points)
                    segment_r_sqs.append((signal_a, signal_b, model, r_sq))
                
                segment_weights = np.array(segment_points_list) / total_points             

                # Weighted mean of R^2 values for each segment
                mean = sum(r_sq_val[-1]*segment_weights[i] for i, r_sq_val in enumerate(segment_r_sqs))
                
                if model is not None:
                    top_matches.append((signal_a, signal_b, model, mean))
    ...
    def signal_match(self, signal_a, signal_b, start_time, end_time):
        ...
        if len(x) >= 2:
            ...
            return model, r_sq, segment_points
        else:
            return None, 0, segment_points
