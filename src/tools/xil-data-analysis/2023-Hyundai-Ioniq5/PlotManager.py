...
class PlotManager:
    ...
    def plot_accel_decel_time_heat_map(self):
        ...
        heatmap_pivot_accel_decel = heatmap_df_accel_decel.pivot_table(values='Accel/Decel Time (s)', 
                                                                    index='Speed Range', 
                                                                    columns='Acceleration (m/s²)').iloc[::-1]
    ...
    def plot_respose_time_surface_plot(self):
        ...
        # Create a meshgrid for surface plotting without interpolation
        ...
        """ 
            - Define grid resolution (higher resolution for smoothness)
            - Use SciPy's griddata for interpolation instead of directly averaging values.
            - Increase grid resolution to create a finer mesh.
            - Apply Gaussian smoothing with scipy.ndimage.gaussian_filter to reduce sharp transitions
        """
        accel_grid = np.linspace(plot_df['Acceleration (m/s²)'].min(), plot_df['Acceleration (m/s²)'].max(), 100)
        speed_grid = np.linspace(plot_df['Speed Numeric'].min(), plot_df['Speed Numeric'].max(), 100)
        X, Y = np.meshgrid(accel_grid, speed_grid)

        # Interpolating the Response Time values
        Z = griddata((plot_df['Acceleration (m/s²)'], plot_df['Speed Numeric']), 
                    plot_df['Response Time (s)'], 
                    (X, Y), method='cubic')

        # Apply Gaussian smoothing
        Z = gaussian_filter(Z, sigma=1)  # Adjust sigma for smoothness
    ...
    def plot_respose_time_contour_plot(self):
        ...
        plot_data = []
        for speed_range, df in {speed: pd.read_excel(xls, sheet_name=speed) for speed in speed_ranges}.items():
            df_cleaned = df[['Accel_Value', 'Response_Time']].dropna()
            for _, row in df_cleaned.iterrows():
                plot_data.append([speed_range, row['Accel_Value'], row['Response_Time']])

        # Convert to DataFrame
        contour_df = pd.DataFrame(plot_data, columns=['Speed Range', 'Acceleration (m/s²)', 'Response Time (s)'])
