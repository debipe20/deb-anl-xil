...
/**
 * This version of QName models the value space for XML Schema QNames, which
 * consists of just a namespace URI and a local name.
 * The name OSXMLQName is already used for a type that consists of prefix and
 * local name, which is the lexical representation.
 */
typedef struct {
   const OSUTF8CHAR* nsURI;      /* use null for empty namespace */
   const OSUTF8CHAR* localName;
} OSXMLQNameValue;
 ...
 * @param pArray Array to search.  It must be sorted by localName then nsURI,
 * with null nsURI (no namespace) being less than non-null nsURI (non empty
 * namespace).
 * @param arraySize Size of the array
 * @return index of matching element.  If multiple elements match pValue, the
 *    index of the smallest such element is returned. If there is no match,
 *    (OSSIZE)-1 is returned.
 */
EXTERNRT OSSIZE rtxQNameValueArraySearch(OSXMLQNameValue* pValue,
   OSXMLQNameValue* pArray, OSSIZE arraySize);
