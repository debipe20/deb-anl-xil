...
VEH_TYPE_DICT = {0: "unknown", 2: "EmergencyVehicle",
                 4: "Car", 6: "Transit", 9: "Truck"}
...
class SimulatedBsmBlobProcessor:
    ...
    def pack_variables_to_blob(self,
        ...
        if type(vehicle_Type) == str and vehicle_Type in VEH_TYPE_DICT.values():
            vehicle_Type = dict(zip(VEH_TYPE_DICT.values(), VEH_TYPE_DICT.keys()))[
                vehicle_Type]
    ...
    def pack_json_to_blob(self, bsmJson: str) -> (bytes or bool):
        ...
        if bsmJson["MsgType"] == "BSM":
            ...
            latitude_DecimalDegree = bsmJson["BasicVehicle"]["position"]["latitude_DecimalDegree"] * \
                MULTIPLIER_LATITUDE
            longitude_DecimalDegree = bsmJson["BasicVehicle"]["position"]["longitude_DecimalDegree"] * \
                MULTIPLIER_LONGITUDE
            elevation_Meter = bsmJson["BasicVehicle"]["position"]["elevation_Meter"] * \
                MULTIPLIER_ELEVATION
            speed_MeterPerSecond = bsmJson["BasicVehicle"]["speed_MeterPerSecond"] * \
                MULTIPLIER_SPEED
            heading_Degree = bsmJson["BasicVehicle"]["heading_Degree"] * \
                MULTIPLIER_HEADING
            length_cm = bsmJson["BasicVehicle"]["size"]["length_cm"] * \
                MULTIPLIER_LENGTH
            width_cm = bsmJson["BasicVehicle"]["size"]["width_cm"] * \
                MULTIPLIER_WIDTH
