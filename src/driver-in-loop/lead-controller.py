...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        if self.hud: self.world.on_tick(hud.on_world_tick)
    ...
    def restart(self):
        ...
        if self.hud: self.hud.notification(actor_type)
    ...
    def next_weather(self, reverse=False):
        ...
        if self.hud: self.hud.notification('Weather: %s' % preset[1])
...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        # world.FEhud.notification("External control active (keyboard disabled).", seconds=4.0)
        if world.hud:
...
class CollisionSensor(object):
    ...
    def _on_collision(weak_self, event):
        ...
        if self.hud: self.hud.notification('Collision with %r' % actor_type)
...
class LaneInvasionSensor(object):
    ...
    def _on_invasion(weak_self, event):
        ...
        if self.hud: self.hud.notification('Crossed line %s' % ' and '.join(text))
    ...
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                ...
                bp.set_attribute('image_size_x', str(hud.dim[0]) if hud else '800')
                bp.set_attribute('image_size_y', str(hud.dim[1]) if hud else '600')
...
def main():
    argparser = argparse.ArgumentParser(description='CARLA Lead Vehicle Controller (No GUI)')
    ...
    try:
        ...
        # No HUD or pygame display
        world = World(client.get_world(), None, args)
        controller = KeyboardControl(world, start_in_autopilot=False)

        print("[INFO] Lead vehicle is ready and under external control.")
        while True:
            time.sleep(0.05)
            controller.parse_events(client, world, None)

    except KeyboardInterrupt:
        print("\n[INFO] Stopping simulation due to user interrupt.")
    finally:
        if controller:
            controller.destroy()
        if world:
