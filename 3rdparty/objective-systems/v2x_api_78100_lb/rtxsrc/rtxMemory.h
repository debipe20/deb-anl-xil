 ...
 *
 * Note: rtxMemHeapConvertStatic is generally preferrable to
 * rtxMemStaticHeapCreate, since in most cases you will already have a heap
 * object that you can reuse.
 *
 * @param ppvMemHeap Pointer-to-pointer to variable to receive created object.
 ...
 * All allocations are done from the static block of memory that is provided.
 * It is much faster than the standard management but has some limitations such
 * as the inability to free individual pointer values.  All memory in the
 * block must be freed at once.
 *
 * If memory was previously allocated using the given heap, it will be
 * freed before conversion.
 *
 * @param ppvMemHeap Pointer-to-pointer to heap variable.  ppvMemHeap and
 * *ppvMemHeap must not be null;  otherwise, an error is returned.
 * @param pmem Pointer to static memory block to use for allocations.
 * @param memsize Sizeof the memory block in bytes.
 * @return Status of the creation operation: 0 = success or < 0 for an error.
 */
