...
#define LIC_RT     0x00
/*
LIC_EXI will only be used by the XBinder run-time, so it can have the same
bit mask as LIC_XER, which is not used by the XBinder run-time.
*/
#define LIC_EXI    0x01
#define LIC_XER    0x01
#define LIC_PRO    0x02
...
#define LCHECKEXI(pctxt) \
{ int lcret = rtxCL(pctxt, LIC_EXI); if (0 != lcret) return lcret; }
