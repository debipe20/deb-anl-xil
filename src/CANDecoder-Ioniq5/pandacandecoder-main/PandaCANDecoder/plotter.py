...
def plot_probability_chart(ax, msg, byte_order):
    ...
    for xc in range(0, 64, 8):
        ...
        if byte_order == 'be':
            ax.text(xc, 1.02, f"Byte {xc//8 + 1}", fontsize=8)
        elif byte_order == 'le':
            ax.text(xc, 1.02, f"Byte {7-(xc//8) + 1}", fontsize=8)
...
def plot_ts_signal(ax, signal, message_start):
    '''
    ...
    ax.set_title(f"Signal: {signal.msg.msg_id} {message_start[2:]}")
...
def plot_ts_signal_comparison(ax, signal_a, signal_b, m, b, r_sq):
    ...
    ax.set_title(f"Signal Match: Time Series Data\nm={m:.2f}, b={b:.2f}, R^2={r_sq:.4f}, Signedness={signal_a.signedness}")
