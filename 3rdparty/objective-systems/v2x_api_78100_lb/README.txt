...
v2x_api_<version>_lb (or _ub or _us)
  +- debug
     +- lib
     +- src
  +- doc
  ...
  +- sample
     +- BasicSafetyMessage
     +- BasicSafetyMessage2020
     +- BasicSafetyMessage2016
     +- CAMMessage
     +- DENMMessage
     +- python
  +- utilsrc
...
The sample/python directory contains a sample Python script.  See the
...
On Mac, the DYLD_LIBRARY_PATH environment variable must be defined
to point at the location in which the shared object file is 
located: 

export DYLD_LIBRARY_PATH=../../debug/lib

In each of these cases, the path to the release library directory 
